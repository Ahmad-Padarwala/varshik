...
const EditPayment = () => {
  ...
  const getPaymentInfo = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_PROXY}/getPaymentInfo/${location.state.paymentId}`)
  ...
  const updatePayment = async (e) => {
    ...
    try {
      ...
      await axios.patch(`${process.env.REACT_APP_PROXY}/updatepayment/${paymentId}`, team).then((res) => {
        console.log(res);
  ...
  const handleAgreeDialog = async (value) => {
    if (value === '1234') {
      try {
        await axios.delete(`${process.env.REACT_APP_PROXY}/deletePayment/${paymentId}`);
        navigate("/paymentHistory")
      } catch (error) {
