...
const AddPayment = () => {
  ...
  const getSinglePaymentData = async () => {
    try {
      await axios
        .get(`${process.env.REACT_APP_PROXY}/getsinglepaymentdata`)
        .then((res) => {
  ...
  const savedata = async (e) => {
    ...
    try {
      ...
      await axios
        .post(
          `${process.env.REACT_APP_PROXY}/addpayment/${memberId}/${rollNo}`,
          team
        )
        .then((res) => {
          console.log(res);
  ...
  const checkMember = async (id) => {
    ...
    try {
      await axios
        .get(`${process.env.REACT_APP_PROXY}/getpermemberbyrollno/${id}`)
        .then((res) => {
            ...
            <div className="sfdropdown">
              {memberList
                ...
                .map((item) => {
                  ...
                  return (
                    <div
                      onClick={() => onSearch(fullName, item.id)}
        ...
        <div class="dash-content mt-5">
          <div>
            <div style={{ marginBottom: "20px" }}>
              <span>
                <b style={{ color: "red" }}>Member No : </b> {payment && payment.member_id}
              </span>
              <span>
                <b style={{ color: "red", paddingLeft: "20px" }}>
                  Full Name :{" "}
                </b>{" "}
                {payment && payment.member_name}
              </span>
              <span>
                <b style={{ color: "red", paddingLeft: "20px" }}>
                  Clan Name :{" "}
                </b>{" "}
                {payment && payment.clan_name}
              </span>

              <span>
                <b style={{ color: "red", paddingLeft: "20px" }}>Amount : </b>{" "}
                {payment && payment.pay_amount}
              </span>
            </div>
          </div>
          <div class="card">
