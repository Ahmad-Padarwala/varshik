...
const EditMember = () => {
  ...
  const getDataPerMember = async () => {
    ...
    try {
      ...
      await axios.get(`${process.env.REACT_APP_PROXY}/getpermember/${location.state.id}`).then((res) => {
  ...
  const updateData = async (e) => {
    ...
    if (memberExists) {
      ...
      try {
        await axios.post(`${process.env.REACT_APP_PROXY}/updateMember`, team).then((res) => {
          console.log(res);
          navigate('/allmember')
        }).catch((e) => {
          console.log(e);
